...
Der Baustein Security bildet eine Komponente des Querschnitts der xref:4.0.x@isyfact-standards-doku:referenzarchitektur:referenzarchitektur-it-system/master.adoc[].
...
Aus den xref:4.0.x@isyfact-standards-doku:referenzarchitektur:software-technisch.adoc#prinzipien-der-sicherheitsarchitektur[Prinzipien der Sicherheitsarchitektur] leiten sich die folgenden Randbedingungen für die Umsetzung der Berechtigungsprüfung innerhalb eines IT-Systems ab:
...
Folglich muss ein IT-System, das ein Nachbarsystem aufruft, diesem Aufruf ebenfalls gültige und valide Authentifzierungs- und Autorisierungsdaten mitgeben.
...
Gemäß Referenzarchitektur ist HTTP als das Übertragungsprotokoll der xref:4.0.x@isyfact-standards-doku:referenzarchitektur:software-technisch.adoc#servicekommunikation[Servicekommunikation] definiert.
...
Gemäß der IsyFact-Systemlandschaft - siehe xref:4.0.x@isyfact-standards-doku:blaupausen:detailkonzept-komponente-batch/master.adoc#grobe-architektur-des-batchrahmens[Grobe Architektur des Batchrahmens] - ist ein Batch Teil eines IT-Systems und setzt sich - siehe xref:image-awlogbat[Fachliche Architektur eines Batch in der Anwendungslandschaft] - aus den Komponenten (1) Batch-Logik und -Steuerung sowie den dabei eingesetzten (2) Fachkomponenten zusammen.
...
image::4.0.x@isyfact-standards-doku:blaupausen:detailkonzept-batch/architektur-it-system-batch-parts.dn.svg[]
...
Zur Unterstützung einer weichen Migration wird mit `IsyFact-3` der xref:resource-owner-pasword-credential-flow[] mit `Benutzer` und `Passwort` weiterhin möglich sein.
...
In `isy-task` wird die übergebene `TaskID` verwendet, um in der Betriebskonfiguration gemäß den xref:4.0.x@isyfact-standards-doku:isy-task:nutzungsvorgaben.adoc#konfigurationsschluessel[Task-Konfigurations-Konventionen] nach den entsprechenden Konfigurationswerten aufzulösen.
...
Die *externe IAM-URI* wird vom Portal als Redirect an den WebClient zurückgeliefert, so dass dieser den IAM-Service direkt von extern aufrufen kann, um den IAM-Login-Dialog im WebClient zur Anzeige zu bringen.
...
Die verschiedenen dargestellten Zugriffswege _Portal_, _Service-Gateway_, _Authentication-Gateway_ oder _interne Anwendungs-Authentifizierung_ (Batch/Task/Kontextwechsel) bis hin zu mehrfachen IAM-Instanzen können daher als einzelne Tenants nicht nur unterschieden, sondern auch als Multi-Instanzen des gleichen Typs identifiziert werden.
...
Die bisher aufgezeigten Authentifizierungswege werden in Bezug auf `isy-security` als jeweilig unterschiedliche Tenants behandelt bzw. konfiguriert.
...
Folgendes Beispiel zeigt die Trennung nach Zugriffswegen (<<image-multi-tenancy-isyfact>>):

_In diesem Beispiel wurde ein weiterer IAM-Service aufgenommen, um die Möglichkeiten zur Nutzung mehrere IAM-Services zu verdeutlichen.
Dieser kann auch außerhalb der Anwendungslandschaft betrieben werden.
Die IsyFact sieht weiterhin die Nutzung eines einzelnen IAM-Service je Anwendungslandschaft vor._
...
* Die interne Batch-Anwendung greift auf die GA zu.
