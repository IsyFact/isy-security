...
* <<absicherung_von_services>>.

Schließlich beschreibt das letzte Kapitel <<testunterstuetzung>>, welche Hilfen der Baustein Security zum Erstellen von (automatisierten) Tests bereitstellt.

Die konzeptionellen Aspekte der Authentifizierung und Autorisierung werden im xref:konzept.adoc[] beschrieben.
...
Im Folgenden werden die xref:konzept.adoc#aussensicht-der-komponente-security[Interfaces des Bausteins Security] genauer erläutert.
...
Authentifiziert einen OAuth 2.0 Client anhand des übergebenen ClientRegistration-Objektes. Unterstützt:
* `client_credentials`-GrantType (_Client Credentials_ Flow).

`void authentifiziere(ClientRegistration clientRegistration, AdditionalCredentials credentials)`::
Authentifiziert einen OAuth 2.0 Client anhand des übergebenen ClientRegistration-Objektes mit zusätzlichen dynamischen Authentifizierungsdaten. Unterstützt:
* `client_credentials`-GrantType: Hier kann ein BHKNZ über die AdditionalCredentials übergeben werden.
...
AdditionalCredentials credentialsWithBhknz = AdditionalCredentials.createWithBhknz("123456");
...
AdditionalCredentials credentialsWithUsernamePassword = AdditionalCredentials.createWithUsernamePassword("user", "passwort");
...
AdditionalCredentials credentialsWithUsernamePasswordBhknz = AdditionalCredentials.createWithUsernamePasswordBhknz(
...
|client.registration._<registrationId>_.authorization-grant-type |String |Methode, durch welche die Anwendung ein Bearer-Token erhält. Abhängig von dem zu verwendenden auf `client_credentials` (Client Credentials Flow) oder `password` (Resource Owner Password Credentials Flow) zu setzen.
...
Authentifizierung per `oauth2ClientRegistrationId`:
Die Auswahl des Flows erfolgt über den `authorization-grant-type` der vollständig konfigurierten `ClientRegistration` (siehe <<listing-client-registrations>>).
...
Spring Security bietet umfassenden Support für `OAuth 2.0` und unterstützt die Authentifizierung auf Basis des `Client Credentials Flow`, wobei auf Spring interne Mechanismen zugegriffen wird.
...
Weitere Informationen und detaillierte Anleitungen zur Verwendung können im Abschnitt zum <<authentifizierungsmanager-interface>> gefunden werden.
...
* xref:isyfact-standards-doku:referenzarchitektur:software-technisch/batch/batchrahmen.adoc#authentifizierung-und-autorisierung[Authentifizierung und Autorisierung von Batches]
* bzw. xref:task:ROOT:nutzungsvorgaben.adoc#absicherung-von-tasks[Absicherung von Tasks]

verwiesen. Diese bieten praxisorientierte Richtlinien zur Gewährleistung der Sicherheit von Batches und Tasks.

[[absicherung_von_services]]
== Absicherung von Services
Spring Security unterstützt die automatische Absicherung von xref:glossary::terms-definitions.adoc#service[Services] einer Anwendung und eine feingranulare Autorisierung im Rahmen von `OAuth 2.0`.
...
[NOTE]
====
Bei der Verwendung von `isy-sicherheit` wurde der CSRF-Filter von Spring Security in vielen Fällen standardmäßig deaktiviert.
Damit die von der Anwendung bereitgestellten Schnittstellen weiterhin ohne Probleme erreicht werden können, muss dieser möglicherweise durch entsprechende Konfiguration der `SecurityFilterChain` wieder deaktiviert werden: `http.csrf().disable()`.
...
.Konfigurationsparameter eines Services
[cols="3m,2m,2m,8",options="header"]
|===
|Parameter |Wertebereich |Default |Beschreibung
...
.Isyfact-spezifische Konfigurationsparameter eines Services
[cols="3m,2m,2m,8",options="header"]
|===
|Parameter |Wertebereich |Default |Beschreibung
...
Der Pfad kann in der `application.properties` oder `application.yaml` der verwendenden Anwendung wie folgt verwendet werden:

.Konfiguration des Pfads in `application.properties`
[source,properties]
----
isy.security.role-privileges-mapping-file=classpath:/<custom-path>/rollenrechte.xml
----

.Konfiguration des Pfads in `application.yaml`
[source,yaml]
----
isy:
  security:
    role-privileges-mapping-file: classpath:/<custom-path>/rollenrechte.xml
----

Der Baustein liefert ein xref:nutzungsvorgaben/anhang.adoc#anhang-rollen-rechte-schema[XML-Schema für den Aufbau der Konfigurationsdatei] mit.
...
[[autorisierung-service]]
=== Autorisierung an einem Service
...
Die Annotation kommt in der Serviceschicht zur Anwendung und autorisiert den Zugriff auf eine Service-Methode.
...
Für Webcontroller ohne Serviceschicht kann https://docs.spring.io/spring-security/reference/servlet/authorization/authorize-http-requests.html[Request Level Autorisierung] genutzt werden.
