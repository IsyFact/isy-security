...
Authentifiziert einen OAuth 2.0 Client anhand seiner Client-Registration-ID. Unterstützt den `client_credentials` (_Client Credentials_ Flow) GrantType.
...
* `bhknz`: Das BHKNZ kann als optionaler Parameter übergeben werden.
...
|client.registration._<registrationId>_.authorization-grant-type |String |Methode, durch welche die Anwendung ein Bearer-Token erhält. Für den Client Credentials Flow auf `client_credentials` zu setzen.
