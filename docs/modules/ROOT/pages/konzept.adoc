...
Mit Einführung des Bausteins `isy-security` wird auch für die Absicherung von _Aufgaben_ der _Credential_-Flow xref:client-credential-flow[] unterstützt.
Das bedeutet, dass die für die _Aufgaben_-Authentifizierung verwendete Parametrisierung per Konfiguration in _isy_task_ nicht mehr wie bisher als _NatürlicherNutzer_ zu `Benutzer` und `Passwort` aufgelöst werden kann, sondern neu als _TechnischerNutzer_ (Client) zu `Client-ID` und `Client-Secret` aufgelöst werden muss.
