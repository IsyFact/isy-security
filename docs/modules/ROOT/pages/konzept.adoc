...
Batches und Tasks sind als Confidential Client eingestuft und verwahren ihre Authentifizierungsdaten (Client-ID u. Client-Secret) in ihrer Konfiguration (mit einer `oauth2ClientRegistrationId` als Schlüssel).
...
Dabei wird anhand des Typs der Authentifizierungsdaten der _Credential_-Flow xref:client-credential-flow[] verwendet:
...
Die Klasse `AdditionalCredentials` dient zur Kapselung eines BHKNZ (Behördenkennzeichen) als zusätzliches Authentifizierungsmerkmal bei der Authentifizierung mit ClientRegistration-Objekten.
Eine Instanz wird über die statische Factory-Methode `createWithBhknz` erstellt, die ein BHKNZ als Parameter erwartet.

zusätzlicher Authentifizierungsdaten
Sie ermöglicht die flexible Kombination verschiedener Credentials wie Benutzername, Passwort und BHKNZ.

Die Klasse bietet zudem die Prüfmethoden `hasBhknz`, um die Verfügbarkeit des BHKNZ zu ermitteln.
...
Für die Authentifizierung und Autorisierung des Batch wird der _Credential_-Flow xref:client-credential-flow[] genutzt.
...
Über die Batch-Konfiguration wird gesteuert, wie der Batch mit einem _Client_ und den zugehörigen Authentifizierungsdaten `Client-ID` und `Client-Secret` über den xref:client-credential-flow[] authentifiziert und autorisiert wird.

Die eigentliche Authentifizierung erfolgt dann mit den erhaltenen Authentifizierungsdaten des Batch-Nutzers durch den IAM-Service der Plattform.
...
Für die Absicherung von _Aufgaben_ wird der _Credential_-Flow xref:client-credential-flow[] verwendet.
Die für die _Aufgaben_-Authentifizierung verwendete Parametrisierung per Konfiguration in _isy_task_ wird als _TechnischerNutzer_ (Client) zu `Client-ID` und `Client-Secret` aufgelöst werden muss.

Mit `IsyFact-5` wird der zuvor verfügbare Resource Owner Password Credential Flow mit `Benutzer` und `Passwort` nicht mehr unterstützt.
...
Innerhalb der IsyFact wird bei der Autorisierung von Anwendungsaufrufen (von intern oder extern) das Verfahren  _Client-Credential_ angewendet, da es sich hierbei ausschließlich um die Berechtigungszuteilung für _Confidential-Clients_ dreht.
