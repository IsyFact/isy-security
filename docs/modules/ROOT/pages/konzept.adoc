...
Der `Authentifizierungsmanager` übernimmt die programmatische Authentifizierung und Autorisierung interner und externer Anwendungen durch das IAM für den Zugang zu IsyFact-Nachbaranwendungen. Er bietet verschiedene Methoden für die Authentifizierung, sowohl über Registration-IDs als auch über direkte ClientRegistration-Objekte.
...
oder in den xref:nutzungsvorgaben.adoc#authentifizierungsmanager-interface[Nutzungsvorgaben] gefunden werden.
...
An diesen Servicepunkt werden von der externen Anwendung die Authentifizierungsdaten übergeben.
...
. Tenant 1: Gruppierung aller Systeme mit Zugriffsweg über das Service-Gateway.
