...
Sicherheit ist ein zentrales Thema einer jeder  xref:glossary::terms-definitions.adoc#glossar-geschaeftsanwendung[Geschäftsanwendung].
...
Das Kapitel <<sicherheitsarchitektur-eines-it-systems>> beleuchtet die Einbettung des Bausteins Security in ein konkretes xref:glossary:glossary:master.adoc#glossar-it-system[IT-System].
Hierbei werden im Abschnitt <<aussensicht-der-komponente-security>> die zur Verfügung gestellten Schnittstellen aufgeführt und der Abschnitt <<aufruf-von-nachbarsystemen>> befasst sich mit Vorgaben und Konventionen beim Einbinden von xref:glossary::terms-definitions.adoc#nachbarsystem[Nachbarsystemen].

Im Kapitel <<oauth2.0-und-openidconnect>> ist das Verfahren zur Authentifizierung und Autorisierung mit OAuth2 und Spring Security beschrieben.
Die Begrifflichkeiten zum Verständnis von OAuth2.0 werden im Abschnitt <<oauth2.0-zur-autorisierung>> erläutert und im Abschnitt <<authentifizierung-und-autorisierung>> wird die Authentifizierung und Autorisierung für Web-Oberflächen, Service-Gateways sowie Batches und Tasks beschrieben.
Im Abschnitt <<token>> wird der Aufbau und die Verwendung von Access-Tokens erklärt.
...
Der Baustein Security bildet eine Komponente des Querschnitts der xref:isyfact-standards-doku:referenzarchitektur:software-technisch/backend.adoc[].
...
Aus den xref:isyfact-standards-doku:referenzarchitektur:software-technisch.adoc#prinzipien-der-sicherheitsarchitektur[Prinzipien der Sicherheitsarchitektur] leiten sich die folgenden Randbedingungen für die Umsetzung der Berechtigungsprüfung innerhalb eines IT-Systems ab:
...
* Anfragen, die an einen Service (von außen über ein Service-Gateway (SGW) oder von anderen IT-Systemen innerhalb der IT-Systemlandschaft) eines IT-Systems eingehen, sind ebenso bereits authentifiziert.
...
image::konzept/software-architektur-berechtigungspruefung.png[align="center"]

Im Folgenden werden die Aufgaben und grobe Funktionsweise der Komponenten für die Autorisierung von Anfragen in einer xref:glossary::terms-definitions.adoc#glossar-geschaeftsanwendung[Geschäftsanwendung] erläutert.
...
Außerdem sichert der Baustein `isy-security` die Services der IsyFact-Anwendungen ab, indem er Funktionalität zur Berechtigungsprüfung anbietet. 
Hier wird geprüft, ob die gewährte Autorisierung auch das Recht zur Nutzung des angefragten Services beinhaltet.
...
Die Authentifizierung (Anmeldung) von internen oder externen Anwendungen verläuft über Service-Methoden, die dieser Manager anbietet.
...
Dieses Access-Token legt die aufrufende Anwendung (bzw. das Service-Gateway als Mittelsmann) anschließend im `SecurityContext` ab und präsentiert es dem aufgerufenen Service der IsyFact-Anwendung.
...
Die Absicherung der Services von IsyFact-Anwendungen erfolgt über die von Spring Security bereitgestellte Annotation `@Secured`.
...
TIP: Die Annotation `@Secured` löst die Annotation `@Gesichert` ab und ist analog dazu verwenden.
...
Hinweis: Die durch Spring Boot 4.1.0 eingeführten Properties authorities-claim-expressions, sowie die bestehenden Properties authorities-claim-name, authorities-claim-delimiter und authority-prefix, greifen nicht, da durch den RolePrivilegeGrantedAuthoritiesConverter eine eigene JwtAuthenticationConverter-Bean instanziiert wird.
...
Er bietet verschiedene Methoden für die Authentifizierung, sowohl über Registration-IDs als auch über direkte ClientRegistration-Objekte.
...
Die Methode `authentifiziere(String oauth2ClientRegistrationId)` im Authentifizierungsmanager liest diese Authentifizierungsdaten aus der betrieblichen Konfiguration und führt damit eine OAuth2.0-konforme Authentifizierung und Autorisierung der Anwendung gegenüber dem IAM-System durch.
...
xref:glossary::terms-definitions.adoc#externes-system[Externe Systeme] übermitteln ihre Authentifizierungsdaten (Client-ID u. Client-Secret) mit im Header der HTTP-Anfrage an das Service-Gateway bzw. Authentication-Gateway.
...
Dabei wird anhand des Typs der Authentifizierungsdaten der jeweils passende Authentication-Flow verwendet:

- Dafür sind folgende Methoden verfügbar:
...
- `createWithUsernamePassword` für Benutzername und Passwort,
- `createWithUsernamePasswordBhknz` für Benutzername, Passwort und BHKNZ, sowie
- `createWithBhknz` nur für das BHKNZ.
...
Ein IT-System erwartet bei einem Aufruf, dass alle notwendigen Authentifizierungs- und Autorisierungsdaten in valider und gültiger Form übergeben werden.
Folglich muss ein IT-System, das ein xref:glossary::terms-definitions.adoc#nachbarsystem[Nachbarsystem] aufruft, diesem Aufruf ebenfalls gültige und valide Authentifizierungs- und Autorisierungsdaten mitgeben.
Im Regelfall werden diese Daten der originären Anfrage als `SecurityContext` übergeben, der mit den Daten aus dem Access-Token zuvor initialisiert, wurde.
Dies geschieht durch die xref:validierung-der-token[Validierung des Access-Tokens] über den Sicherheitsbaustein `isy-security` des IT-Systems.
...
Gemäß Referenzarchitektur ist HTTP als das Übertragungsprotokoll der xref:isyfact-standards-doku:referenzarchitektur:software-technisch.adoc#servicekommunikation[Servicekommunikation] definiert.
...
Hierbei muss das aufrufende IT-System stets das `Access-Token` manuell in den Header der ausgehenden Nachricht schreiben.
...
*Ressourcen-Server bzw. IT-System (Resource Server)*
...
Wir übernehmen für die IsyFact die IsyFact-spezifische Bezeichnung "IT-System".
...
Dieses neue Access-Token kann zum Beispiel ein späteres Ablaufdatum (automatische Nutzungsverlängerung) oder andere Rechte (innerhalb aller an den Ressourcenbesitzer vergebenen Rechte) aufweisen.  +
...
Damit ein Anwender eine geschützte Anwendung über deren Web-Oberfläche (im Webbrowser) aufrufen kann, muss er über den IAM-Service authentifiziert und dazu autorisiert sein.
...
Dies kann ein als Reverse Proxy konfigurierter Webserver sein (z.B. Apache http-Server mit `mod_auth_openidc` Modul), der zwischen  Web-Client (Webbrowser) und Web-Anwendung geschaltet ist.
...
Die folgende Abbildung zeigt die System-Architektur mit dem Webserver in der Informations- und Dienstezone vor der Anwendung und dem IAM-Service in der Logik- u. Verarbeitungszone.
...
Ein menschlicher Nutzer, der eine Anwendung über deren Web-Oberfläche (im Webbrowser) bedienen möchte, ist im Besitz von Authentifizierungsdaten (Nutzername u. Passwort), mit denen er sich gegenüber dem IAM-System ausweisen kann.
...
Anfragen von _externen IT-Systemen_ an ein IsyFact-IT-System, müssen über den IAM-Service authentifiziert und zur Nutzung des aufgerufenen IsyFact-Service autorisiert werden.
...
. Mit Erhalt eines gültigen Access-Tokens leitet das Service-Gateway die fachliche Anfrage zusammen mit dem Token an das eigentliche IT-System weiter.
In der Anwendung kann anschließend die Ausführungsberechtigung des Benutzers verifiziert werden.

Dabei muss die Authentifizierung der externen Anwendung nicht gemäß _SpringSecurity (OAuth2.0)_ auf einen _OAuth2.0 Client_ (mit Client-ID u. Client-Secret) migriert worden sein,
...
Die Authentifizierung über das Service-Gateway stellt einen wichtigen Schritt in der Sicherung von Anwendungen dar.
...
Weitere Informationen und detaillierte Anleitungen zur Verwendung der `authentifiziere`-Methode können in der entsprechenden Dokumentation des Authentifizierungsmanagers oder in den xref:nutzungsvorgaben.adoc#authentifizierungsmanager-interface[Nutzungsvorgaben] gefunden werden.
...
Die Authentifizierung wird von der eigentlichen fachlichen Anfrage getrennt, sodass sie nur einmal durchgeführt werden muss und das erhaltene Access-Token (innerhalb seiner Gültigkeit) auch für die weiteren fachlichen Anfragen verwendet werden kann.

[[aufruf-service-gateway-mit-authentication-gateway]]
===== Direkte Authentifizierung am IAM-Service

Damit eine externe Anwendung auf die Ressourcen eines aufgerufenen IT-Systems zugreifen kann, benötigt sie ein gültiges Access-Token.
...
Zukünftig sollen die Aufrufe des IAM-Service direkt aus der externen Anwendung über ein Authentication-Gateway erfolgen.
...
. Für die Authentifizierung einer externen Anwendung existiert das _Authentication-Gateway_.
Dort werden von der externen Anwendung die Authentifizierungsdaten übergeben.
...
. Die externe Anwendung kann dieses Access-Token speichern und es anschließend für _mehrere_ fachliche Anfragen an das IT-System (über das _Service-Gateway_) verwenden.
. Das Service-Gateway leitet die fachliche Anfrage zusammen mit dem Access-Token an das aufzurufende IT-System weiter.
Im aufgerufenen IT-System wird anschließend das jeweils mitgelieferte Access-Token validiert.
...
Dabei wird ausschließlich der _Client-Credential-Flow_ mittels einer signierten JWT-Client-Assertion nach dem Verfahren `client_secret_jwt` zur Authentifizierung eines technischen Clients unterstützt.
...
Gemäß der IsyFact-Systemlandschaft - siehe xref:isyfact-standards-doku:referenzarchitektur:software-technisch/batch/batchrahmen.adoc#grobe-architektur-des-batchrahmens[Grobe Architektur des Batchrahmens] - ist ein Batch Teil eines IT-Systems und setzt sich - siehe <<image-awlogbat,Fachliche Architektur eines Batch in der Anwendungslandschaft>> - aus den Komponenten (1) Batch-Logik und -Steuerung sowie den dabei eingesetzten (2) Fachkomponenten zusammen.
...
image::isyfact-standards-doku:referenzarchitektur:software-technisch/batch/architektur-batch-parts.dn.svg[]

Ein Batch wird über einen Batch-Client oder ein Startprogramm – meist ein Shellscript, UC4, JobNetz, CronJob, oder ähnliches – initialisiert und gestartet.
Diese Batch-Clients sind systemseitig dazu berechtigt, den dedizierten Batch auszuführen und damit zu starten.
Somit ruft der Batch-Client die Komponente _Batch-Logik_ des Batches auf, welche die notwendigen Initialisierungen vornimmt.
...
image::konzept/Sequenzdiagramm_AbsicherungBatch.dn.svg[align="center"]
...
Je nach verwendetem _Credential_-Flow erfolgt dann mit den jeweiligen erhaltenen Authentifizierungsdaten des Batch-Nutzers die eigentliche Authentifizierung durch den IAM-Service der Plattform.
...
Auch für die Absicherung von _Aufgaben_ werden beide _Credential_-Flows xref:client-credential-flow[] und xref:resource-owner-password-credential-flow[] unterstützt.
Daher kann die zur _Aufgaben_-Authentifizierung verwendete Parametrisierung per Konfiguration in `isy_task` einerseits als _Natürlicher Benutzer_ zu `Benutzer` und `Passwort` und andererseits als _TechnischerNutzer_ (Client) zu `Client-ID` und `Client-Secret` aufgelöst werden.
...
image::konzept/Sequenzdiagramm_AbsicherungTask.dn.svg[align="center"]
...
In `isy-task` wird die übergebene `TaskID` verwendet, um in der Betriebskonfiguration gemäß den xref:task::nutzungsvorgaben.adoc#konfigurationsschluessel[Task-Konfigurations-Konventionen] nach den entsprechenden Konfigurationswerten aufzulösen.
...
Mit SpringSecurity (OAuth2.0) werden die Berechtigungszuteilungsverfahren (Credential Flows) _Authorization-Code-Credentials_, _Implicit-Credentials_
und _Client-Credentials_ spezifiziert.
Zusätzlich wird durch _isy-security_ auch weiterhin der Credential Flow _Resource-Owner-Password-Credential_ unterstützt.
...
So wird die Authentifizierung für einen technischen Benutzer über diesen Authorization-Flow abgebildet.
...
Hierbei wird die Konfiguration des technischen Benutzers als der _Resource-Owner_ dargestellt.

Einem _Confidential Client_ wird die Konfiguration (1.) eines technischen Benutzers mit den Authentifizierungsdaten `Username` und `Passwort` zugeordnet.
Dieser technische Benutzer muss mit seinen Rollen im Benutzerverzeichnis vorhanden und gepflegt sein.
...
Im IAM erfolgt mit den so übergebenen Authentifizierungsdaten die Authentifizierung sowohl des _Confidential Client_, als auch des technischen Benutzers als _Resource-Owner_, die im Ergebnis ein gültiges Access-Token mit den Rollen des technischen Benutzers als Antwort (3.) zurückgeliefert.

Die Client-Anwendung kann dann, mit dem so erhaltenen Access-Token und in den Rollen des technischen Benutzers, ihre Fachanwendungen aufrufen.
...
IsyFact verwendet vom IAM-Service signierte JWT Access-Token, wie in https://www.oauth.com/oauth2-servers/access-tokens/self-encoded-access-tokens/[OAuth: Self-Encoded Access Tokens] beschrieben.
Zur Prüfung der Echtheit muss die Signatur von dem IsyFact-IT-System verifiziert werden, wie in https://datatracker.ietf.org/doc/html/rfc9068#name-validating-jwt-access-token[RFC 9068: Validating JWT Access Tokens] beschrieben.
...
Falls gefordert ist, auch vorzeitig abgelaufene Access-Token (z.B. durch globales Logout des Benutzers oder Rücknahme (Revoke) der Legitimation des Benutzers) zu erkennen und zu sperren, kann das IsyFact-IT-System das Access-Token zur Prüfung an den IAM-Service schicken, wie in https://datatracker.ietf.org/doc/html/rfc7662[RFC 7662: OAuth 2.0 Token Introspection] beschrieben.


Um auf eine geschützte Ressource eines anderen IT-Systems (Ressourcen-Server) zugreifen zu können, muss die Anwendung in der Anfrage (Request) das Access-Token mitgeben.
...
https://datatracker.ietf.org/doc/html/rfc6750[RFC 6750 Authorization Bearer Token]).
...
Eine eventuelle nochmalige Einreichung desselben Refresh-Tokens bedeutet, dass sich unbefugte Dritte dieses Refresh-Token aneignen konnten.
Das wird durch die 'Automatic Reuse Detection' im IAM-System erkannt und Gegenmaßnahmen ergriffen.

Die Ablaufzeit des Refresh-Tokens bestimmt, wie lange der Benutzer an der Anwendung angemeldet ist und mit ihr arbeiten kann, ohne sich neu authentifizieren zu müssen.
...
Wie in <<aussensicht-der-komponente-security>> beschrieben, wird bei der Authentifizierungsmethode `authentifiziere()` das Behördenkennzeichen, sowie Username und Password in den `AdditionalCredentials` als optionaler Parameter an den `Authentifizierungsmanager` übergeben, der es mit den anderen Authentifizierungsdaten zusammen an den IAM-Service weiterleitet.
...
Dies ist zum Beispiel der Fall, wenn eine interne URI des IAM-Services für Aufrufe von innerhalb der Anwendungslandschaft und eine externe URI für Aufrufe von außerhalb der Anwendungslandschaft benötigt wird oder wenn sich die URIs aufgrund verschiedener Netzwerkzonen bzw. technischer Kommunikationskanäle unterscheiden (müssen).
Der Baustein `isy-security` bietet Unterstützung, um die Nutzung unterschiedlicher URIs für den IAM-Service zu ermöglichen.
Die nachfolgende Skizze zeigt dies schematisch auf.

.schematische Skizze mehrere URIs des IAM-Services
[id="image-multiple-uris",reftext="{figure-caption} {counter:figures}"]
image::konzept/unterschiedliche-issuerURIs.dn.svg[align="center"]

Die *externe IAM-URI* wird vom Portal als Redirect an den WebClient zurückgeliefert, sodass dieser den IAM-Service direkt von extern aufrufen kann, um den IAM-Login-Dialog im WebClient zur Anzeige zu bringen.
Nach erfolgreicher Authentifizierung wird die *externe IAM-URI* in den Access-Token (`iss-Claim`) geschrieben.
Die *interne IAM-URI* wird von der Geschäftsanwendung benötigt, um direkt auf *internem* Wege mit dem IAM zu kommunizieren, z.B. zum Laden von Zertifikatsinformationen.

Die Geschäftsanwendung benötigt in ihrer Konfiguration alle _IAM-URIs_, um alle Access-Token validieren zu können.
Dazu gehört die zum ursprünglichen IAM-Aufruf verwendete URI (`issuer-uri`), die gegen den Wert im `iss-Claim` des Tokens verifiziert wird.
Die Prüfung schlägt fehl, wenn das Access-Token über die *externe IAM-URI* ausgestellt wurde, in der Geschäftsanwendung jedoch (nur) die *interne IAM-URI* zum Nachladen von Zertifikaten konfiguriert ist.
...
Dies geschieht über die Anwendungskonfiguration.
Auf diese Weise können beliebig viele IAM-URIs konfiguriert werden.

Weitere Details zur Konfiguration mehrerer URIs eines IAM wird in den xref:nutzungsvorgaben.adoc#konfiguration-von-multi-iam-uris[Nutzungsvorgaben - Konfiguration von mehreren IAM-URIs] beschrieben.

*Konfigurationsdetails*

Zur weiteren Vertiefung dieser Sonderkonstellation kann in der https://docs.spring.io/spring-security/reference/servlet/oauth2/resource-server/jwt.html#oauth2resourceserver-jwt-jwkseturi[_Spring_-Dokumentation] und für Details bei der Implementierung bzw. Konfiguration kann in den xref:nutzungsvorgaben.adoc#spring-konfiguration[Nutzungsvorgaben] nachgeschlagen werden.


[[multi-tenancy]]
=== Unterstützung von Multi-Tenancy

Der Baustein `isy-security` unterstützt Multi-Tenancy, sodass eine Trennung von Benutzerarten, Zugriffswegen oder IAM-Instanzen in sogenannte Tenants möglich ist.
Beispiele für eine Trennung nach:
...
Wichtig ist, dass alle Systeme, die entlang eines Zugriffsweges aufgerufen werden, jeweils für sich die tenant-spezifische Konfiguration benötigen.
Dies bedeutet zum Beispiel, dass einer Geschäftsanwendung, die ein  Access-Token von einem speziellen Tenant bei der Autorisierung validieren möchte, die Konfiguration des Tenants in der anwendungseigenen Konfiguration vorliegen muss.
Beim Einsatz von Multi-Tenancy ist daher genau zu planen, welcher Tenant (bzw. für welchen Tenant ausgegebene Access-Tokens) Zugriff auf welche Anwendung ermöglichen sollen.
Bei _n_ Benutzergruppen müssen dann bis zu _n_ unterschiedliche Tenant-Konfigurationen in der Anwendungskonfiguration existieren.
Eine detaillierte Beschreibung anhand eines Beispiels findet sich im nächsten Kapitel.

*Für IsyFact-Anwendungslandschaften werden mindestens folgende zwei Tenants für den Zugriff von außerhalb der Anwendungslandschaft gefordert:*

. Tenant 1: Gruppierung aller Systeme mit Zugriffsweg über ein Service-Gateway.
. Tenant 2: Gruppierung aller Anwender mit Zugriffsweg über das Portal.

Hintergrund: Ein Benutzer darf nur über einen festgelegten Zugriffsweg Zugriff auf die Anwendungslandschaft erhalten.
Ein Anwender darf nur über das Portal auf die Anwendungslandschaft zugreifen, nicht aber über ein Service-Gateway.
Bei technischen Nutzern (Systemen) ist dies genau umgekehrt.

[[multi-tenancy-isyfact]]
==== Umsetzung für mehrere Zugriffswege

.Authentifizierungswege als Tenants
[id="image-multi-tenancy-multi-iam",reftext="{figure-caption} {counter:figures}"]
image::konzept/security_multi-tenancy_with_multi_iams.dn.svg[align="center"]

Die verschiedenen dargestellten Zugriffswege _Portal_, _Service-Gateway_, _Authentication-Gateway_ oder _Batch/Task_ bis hin zu mehrfachen IAM-Instanzen können daher als einzelne Tenants nicht nur unterschieden, sondern auch als Multi-Instanzen des gleichen Typs identifiziert werden.

Die an den Zugriffswegen beteiligten Komponenten unterscheiden sich in ihrem Aufbau, ihrer Konfiguration und ihrer Funktionalität, sodass jede genau die speziellen Eigenheiten - siehe dazu Kapitel <<aufruf-weboberflaeche-portal>>, <<aufruf-service-gateway-ohne-authentication-gateway>>, <<aufruf-service-gateway-mit-authentication-gateway>> und <<ausfuehrung-batch-u-task>> bzw. <<aufruf-von-nachbarsystemen>> - am besten bedienen oder erfüllen kann.
Die Umsetzung und sicherheitsrelevante Trennung dieser Zugriffswege werden als eigenständige Tenants betrachtet und können somit auch als mehrere Instanzen pro Zugriffsweg eingerichtet werden.
Dadurch ist es möglich fachliche Unterschiede über den gleichen technischen Zugriffsweg bei der Authentifizierung zu machen.

Die bisher aufgezeigten Authentifizierungswege werden in Bezug auf `isy-security` als jeweils unterschiedliche Tenants behandelt beziehungsweise konfiguriert.
Aber auch pro Authentifizierungsweg ist es möglich unterschiedliche Tenants sprich mehrere Instanzen des gleichen Typs einzurichten.

.Trennung nach Authentifizierungswegen
====
In diesem Beispiel wurde ein weiterer IAM-Service aufgenommen, um die Möglichkeiten zur Nutzung mehrerer IAM-Services zu verdeutlichen.
Dieser kann auch außerhalb der Anwendungslandschaft betrieben werden.
Die IsyFact sieht weiterhin die Nutzung eines einzelnen IAM-Services je Anwendungslandschaft vor.

*Ausgangssituation*

* Interne und externe Anwender werden Tenant 1 (T 1) zugewiesen.
Ihnen steht der Zugang über das Portal zur Verfügung.

* Externen Anwendungen wird der Tenant 2 (T 2) zugewiesen.
Der Zugriff auf die Anwendungslandschaft erfolgt über das SGW.

* Batch-Anwendungen und Tasks wird der Tenant 3 (T 3) zugewiesen.
...
* Die Anwender greifen über das Portal auf die GA zu.
Daher muss in der GA der Tenant 1 (T 1) konfiguriert sein.

* Die externen Anwendungen greifen über das SGW auf die GA zu.
Daher muss in der GA auch der Tenant 2 (T 2) konfiguriert sein.

* Die Batch-Anwendung greift auf die GA zu.
Daher muss in der GA auch der Tenant 3 (T 3) konfiguriert sein.
...
.Beispiel: Trennung nach Authentifizierungswegen
[id="image-multi-tenancy-isyfact",reftext="{figure-caption} {counter:figures}"]
image::konzept/multi_tenancy_isyfact.dn.svg[align="center"]
====

[[rollen-und-rechte]]
== Rollen & Rechte

Die Vergabe von Rollen ist _das_ Mittel der Benutzeradministration, um Anwender der xref:glossary::terms-definitions.adoc#glossar-anwendungslandschaft[Anwendungslandschaft] mit Berechtigungen auszustatten.
...
Es ist konzeptionell beabsichtigt, dass die Administration per Rollen recht grob granular erfolgt.
...
Grob granular wird die Rolle `AnwendungX_Administrator` einem Anwender zugeordnet.
...
Technische Rollen können fachlichen Rollen allerdings untergeordnet werden (siehe <<untergeordnete-rollen,untergeordnete Rollen>>).
...
Dies ist z.B. immer dann notwendig, wenn ein Anwendungsfall die Services eines xref:glossary::terms-definitions.adoc#nachbarsystem[Nachbarsystems] verwendet.
Somit muss im Rahmen des Anwendungsfalls der Service des Nachbarsystems aufgerufen werden.
Die dazu benötigte, technische Rolle muss der fachlichen Rolle untergeordnet werden, damit dies funktioniert.

*Sichtbarkeit der Rolle:* Die Sichtbarkeit der Rollen bei der Zuordnung an Anwender und Systeme kann eingeschränkt werden, um die Administration zu vereinfachen.

Die meisten Rollen sind fachlicher Natur.
Technische Rollen treten oft im Rahmen von Services auf.
Bietet eine Geschäftsanwendung Funktionalität über Services an, so ist die Nutzung jedes Services zumindest durch eine technische Rolle abzusichern.
...
Anwender gelangen in der Regel entweder über das xref:glossary::terms-definitions.adoc#glossar-portal[Portal] oder das xref:glossary::terms-definitions.adoc#glossar-service-gateway[Service-Gateway] in eine Anwendungslandschaft.
...
Für xref:glossary::terms-definitions.adoc#glossar-service-fachlich[Services] des Querschnitts, die nahezu alle Aufrufe benötigen und die keine sicherheitskritischen Operationen anbieten, kann eine zentrale Rolle (z.B. `Querschnitt_Nutzer`) angelegt werden.
...
Fachliche Rollen werden für Anwendungen oder deren Bestandteile vergeben, sofern sie eine Autorisierung der Anfrage vornehmen (grafische Oberflächen und Service-Gateways) bzw. sich selbst autorisieren müssen (Batches und Systemtasks).
...
Der fachliche Systemname beschreibt die Geschäftsanwendung, bzw. die Domäne, in welcher die entsprechende Funktionalität bereitgestellt wird.
...
xref:glossary::terms-definitions.adoc#glossar-it-system[IT-Systeme] werden intern in Form von Batches oder Timer-Tasks aktiv.
