...
Weitere Informationen und detaillierte Anleitungen zur Verwendung können im Abschnitt zum xref:nutzungsvorgaben/inhalt.adoc#authentifizierungsmanager-interface[Authentifizierungsmanager] gefunden werden.
...
* xref:isyfact-standards-doku:blaupausen:detailkonzept-komponente-batch/inhalt.adoc#authentifizierung-und-autorisierung[Authentifizierung und Autorisierung von Batches]
* bzw. xref:4.0.x@isyfact-standards-doku:isy-task:nutzungsvorgaben.adoc#absicherung-von-tasks[Absicherung von Tasks]
